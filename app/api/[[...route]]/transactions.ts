...
import{ addDays, parse, subDays, subWeeks } from "date-fns"
import { transactions, inserttransactionsSchima, catigories, accounts } from '@/db/schema'
...
const app = new Hono()
    ...
    zValidator("query", z.object({
     ...
     async (c) => {
        ...
        const defaultFrom = subWeeks(defaultTo, 30)
        ...
        const data = await db
            .select({
                ...
                catigorieId: transactions.catigorieId,
                catigorie: catigories.name,
            ...
            .leftJoin(catigories, eq(transactions.catigorieId, catigories.id))
            .where(
                and(
                    accountId? eq(transactions.accountId, accountId) : undefined,
                    eq(accounts.userId, auth.userId),
                    gte(transactions.date, startDate),
                    lte(transactions.date, fixedEndDate),
                )
            )
    ...
    .post(
        "/",
        clerkMiddleware(),
        zValidator("json", inserttransactionsSchima.omit({
            id: true,
        })),
    ...
    .patch( "/:id", clerkMiddleware(),
        ...
        async (c) => {
            ...
            const transactionToUpdate = db.$with('update').as(
                db.select({ id: transactions.id }).from(transactions)
                .innerJoin(accounts, eq(transactions.accountId, accounts.id))
                .where(
                    and(
